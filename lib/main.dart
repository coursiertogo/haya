            ...
            Container(
              ...
              child: Row(
                children: [
                  _OperateurTab(
                    label: 'Tmoney',
                    isActive: _operateurSelectionne == 'tmoney',
                    couleur: const Color(0xFF3C3489),
                    onTap: () => setState(() => _operateurSelectionne = 'tmoney'),
                  ),
                  _OperateurTab(
                    label: 'Flooz',
                    isActive: _operateurSelectionne == 'flooz',
                    couleur: const Color(0xFF854F0B),
                    onTap: () => setState(() => _operateurSelectionne = 'flooz'),
                  ),
                ],
              ),
            ),
              ...
              child: Column(
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: _operateurSelectionne == 'tmoney'
                          ? const Color(0xFFEEEDFE) : const Color(0xFFFAEEDA),
                      borderRadius: BorderRadius.circular(20),
                    ),
                    child: Text(_nomOperateur,
                        style: TextStyle(
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Montre ce QR code ou partage ton numéro pour recevoir de l\'argent directement sur ton compte $_nomOperateur.',
...
// Tab sélection opérateur
class _OperateurTab extends StatelessWidget {
  final String label;
  final bool isActive;
  final Color couleur;
  final VoidCallback onTap;
  const _OperateurTab({required this.label, required this.isActive,
...
class _QRCodePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Générateur pseudo-QR basé sur le numéro
    final hash = data.codeUnits.fold(0, (a, b) => (a * 31 + b) & 0xFFFFFF);

    // Coins QR (carrés de positionnement)
    ...
    drawCorner(0, 0);   // haut-gauche
    drawCorner(14, 0);  // haut-droite
    drawCorner(0, 14);  // bas-gauche

    // Points de données (pseudo-aléatoires mais déterministes)
    for (int i = 0; i < 21; i++) {
      for (int j = 0; j < 21; j++) {
        // Éviter les zones des coins
        if ((i < 8 && j < 8) || (i > 12 && j < 8) || (i < 8 && j > 12)) continue;
        // Ligne de synchronisation
        if (i == 6 || j == 6) {
          if ((i + j) % 2 == 0) canvas.drawRect(
              Rect.fromLTWH(i * cell + 1, j * cell + 1, cell - 2, cell - 2), paint);
          continue;
        }
        // Données pseudo-aléatoires
        final v = (hash >> ((i * 21 + j) % 24)) & 1;
        if (v == 1) canvas.drawRect(
            Rect.fromLTWH(i * cell + 1, j * cell + 1, cell - 2, cell - 2), paint);
      }
    }
  }
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            const SizedBox(height: 8),
            const Text('Numéro du bénéficiaire',
            ...
            if (_operateur == 'tmoney')
              _OperateurBox(nom: 'Mixx by Yas (Tmoney)', sub: 'Yas Togo',
                  couleur: const Color(0xFFEEEDFE), bordure: const Color(0xFFAFA9EC),
                  textColor: kNuit, logo: 'M'),
            if (_operateur == 'flooz')
            ...
            if (_operateur == 'inconnu')
              Container(
                ...
                decoration: BoxDecoration(color: const Color(0xFFFEF0F0),
                    borderRadius: BorderRadius.circular(10),
            ...
            Container(
              ...
              child: Row(children: [
                const Padding(padding: EdgeInsets.symmetric(horizontal: 12),
                    child: Text('FCFA', style: TextStyle(fontSize: 13,
                        fontWeight: FontWeight.w500, color: Colors.grey))),
                Expanded(child: TextField(
            ...
            Wrap(
              spacing: 8,
              children: [1000, 2000, 5000, 10000, 25000].map((v) => GestureDetector(
                onTap: () => setState(() => _amountCtrl.text = v.toString()),
                child: Chip(
                  label: Text(v.toString().replaceAllMapped(
...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  final List<Map<String, dynamic>> _transactions = [
    {'initiales': 'AK', 'nom': 'Ama Kpodo', 'op': 'Tmoney', 'date': 'Auj. 10:24', 'montant': '−5 000', 'out': true, 'ci': 0},
    {'initiales': 'YB', 'nom': 'Yawa Bossa', 'op': 'Flooz', 'date': 'Hier · 14:05', 'montant': '+20 000', 'out': false, 'ci': 1},
    {'initiales': 'KD', 'nom': 'Kofi Dossou', 'op': 'Tmoney', 'date': '5 avr.', 'montant': '−10 000', 'out': true, 'ci': 4},
    {'initiales': 'EK', 'nom': 'Edem Klu', 'op': 'Flooz', 'date': '3 avr.', 'montant': '+50 000', 'out': false, 'ci': 5},
    {'initiales': 'NA', 'nom': 'Nana Agbeko', 'op': 'Tmoney', 'date': '1 avr.', 'montant': '−7 500', 'out': true, 'ci': 2},
    {'initiales': 'PK', 'nom': 'Papa Kojo', 'op': 'Flooz', 'date': '29 mars', 'montant': '−15 000', 'out': true, 'ci': 3},
  ];

  List<Map<String, dynamic>> get _filtered {
    if (_filtre == 'tout') return _transactions;
    if (_filtre == 'tmoney') return _transactions.where((t) => t['op'] == 'Tmoney').toList();
    if (_filtre == 'flooz') return _transactions.where((t) => t['op'] == 'Flooz').toList();
    if (_filtre == 'envois') return _transactions.where((t) => t['out'] == true).toList();
    if (_filtre == 'recus') return _transactions.where((t) => t['out'] == false).toList();
