...
Color kFondCtx(BuildContext context) =>
    Theme.of(context).brightness == Brightness.dark ? kFondDark : kFond;
Color kCardCtx(BuildContext context) =>
    Theme.of(context).brightness == Brightness.dark ? kCardDark : Colors.white;
Color kTextCtx(BuildContext context) =>
    Theme.of(context).brightness == Brightness.dark
    ? Colors.white
    : Colors.black87;
Color kSubtextCtx(BuildContext context) =>
    Theme.of(context).brightness == Brightness.dark
    ? Colors.white60
    : Colors.grey;
Color kBorderCtx(BuildContext context) =>
    Theme.of(context).brightness == Brightness.dark
    ? Colors.white12
    : Colors.grey.shade200;
Color kInputCtx(BuildContext context) =>
    Theme.of(context).brightness == Brightness.dark
...
class PinManager {
  ...
  static void definirPin(String pin) {
    _pin = pin;
    _pinDefini = true;
  }
...
class _PinScreenState extends State<PinScreen>
  ...
  @override
  Widget build(BuildContext context) {
    final titre = widget.modeDefinition && _confirming
        ? 'Confirmer le PIN'
        : widget.titre;
                ...
                children: List.generate(
                  ...
                  (i) => Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      color: i < _pin.length
                          ? kOrange
                          : (_error ? kRouge : Colors.white.withOpacity(0.3)),
                    ),
                  ),
                ),
              ),
    ...
    showModalBottomSheet(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setM) {
          final op = numCtrl.text.length >= 2
              ? detectOperateur(numCtrl.text)
              : '';
          return Padding(
      ...
      body: ContactsManager.contacts.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, i) {
                ...
                final tc =
                    avatarTextColors[c.colorIndex % avatarTextColors.length];
                return Container(
...
class _QRCodePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void corner(double x, double y) {
      ...
      canvas.drawRect(
        Rect.fromLTWH((x + 1) * c, (y + 1) * c, 5 * c, 5 * c),
        bg,
      );
    ...
    for (int i = 0; i < 21; i++)
      for (int j = 0; j < 21; j++) {
        ...
        if (i == 6 || j == 6) {
          if ((i + j) % 2 == 0)
            canvas.drawRect(
              Rect.fromLTWH(i * c + 1, j * c + 1, c - 2, c - 2),
              p,
            );
          continue;
        }
        if ((hash >> ((i * 21 + j) % 24)) & 1 == 1)
          canvas.drawRect(Rect.fromLTWH(i * c + 1, j * c + 1, c - 2, c - 2), p);
      }
  }
...
class _PaymentRequestScreenState extends State<PaymentRequestScreen> {
  ...
  @override
  Widget build(BuildContext context) {
    final eur = _montant > 0
        ? TauxChangeService.fcfaVersEuros(_montant)
        : '0.00';
    return Scaffold(
      backgroundColor: kFondCtx(context),
      appBar: AppBar(
                ...
                children: [
                  ...
                  Expanded(
                    child: TextField(
                      ...
                      keyboardType: TextInputType.phone,
                      maxLength: 8,
                      style: TextStyle(fontSize: 16, color: kTextCtx(context)),
                      decoration: const InputDecoration(
                        hintText: 'XX XX XX XX',
                        border: InputBorder.none,
                        counterText: '',
...
class _SendScreenState extends State<SendScreen> {
  ...
  void _confirmerPin() => Navigator.push(
    context,
    MaterialPageRoute(
      builder: (_) => PinScreen(
  ...
  Future<void> _executer() async {
    ...
    final result = await PayGateService.initierPaiement(
      ...
      montant: _montant,
      reseau: PayGateService.convertirOperateur(_op),
      reference: ref,
    );
    Navigator.pop(context);
    if (result['success']) {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const Center(
              child: Text(
                'Securise par PayGate Global · Togo',
...
class _ConfirmationCountdownDialogState
  ...
  Widget build(BuildContext context) {
    ...
    final montantFmt = widget.montant.toString().replaceAllMapped(
      RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'),
      (m) => '\${m[1]} ',
    );
    return Dialog(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Envoi automatique dans $_secondes sec...',
              style: const TextStyle(fontSize: 12, color: Colors.grey),
            ),
            const SizedBox(height: 16),
            SizedBox(
...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  List<Map<String, dynamic>> get _filtered {
    ...
    if (_tri == 'nom')
      ...
      l.sort((a, b) => (b['dv'] as DateTime).compareTo(a['dv'] as DateTime));
    return l;
  }
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 8),
                _TriBtn('Date', 'date', _tri, (v) => setState(() => _tri = v)),
                const SizedBox(width: 6),
                _TriBtn(
                  'Montant',
                  'montant',
          ...
          Container(
            ...
            child: _tri == 'nom'
                ...
                ? TextField(
                    controller: _montantCtrl,
                    onChanged: (v) => setState(() => _montantQ = v),
                    keyboardType: TextInputType.number,
                    style: TextStyle(fontSize: 14, color: kTextCtx(context)),
                    decoration: InputDecoration(
                            ...
                            child: Row(
                              children: [
                                Icon(
                                  ...
                                  size: 16,
                                  color: _debut != null ? kNuit : Colors.grey,
                                ),
                                const SizedBox(width: 8),
                                Text(
                                  _debut != null
                            ...
                            child: Row(
                              children: [
                                Icon(
                                  Icons.calendar_today_outlined,
                                  size: 16,
                                  color: _fin != null ? kNuit : Colors.grey,
                                ),
                                const SizedBox(width: 8),
                                Text(
                                  _fin != null
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(20),
          border: Border.all(color: a ? kOrange : kBorderCtx(context)),
        ),
        child: Text(
          label,
          style: TextStyle(
          ...
          Container(
            ...
            child: Column(
              children: [
                CircleAvatar(
                  radius: 38,
                  backgroundColor: kOrange.withOpacity(0.8),
                  child: const Text(
                    'KA',
                    style: TextStyle(
                      color: Colors.white,
            ...
            child: ListView(
              ...
              children: [
                ...
                _ProfilRow(
                  Icons.euro_outlined,
                  'Taux EUR/FCFA',
                  '1 EUR = ${TauxChangeService.tauxEuroFcfa.toStringAsFixed(2)} FCFA',
                ),
                const SizedBox(height: 18),
                const Text(
                  'Securite',
                ...
                GestureDetector(
                  onTap: () => Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (_) => PinScreen(
                        titre: 'Definir le PIN',
                        sousTitre:
                            'PIN a 4 chiffres pour securiser\nvos transferts',
                        onSuccess: (_) {
                          Navigator.pop(context);
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text('PIN mis a jour !'),
                              backgroundColor: kVert,
                              behavior: SnackBarBehavior.floating,
                            ),
                          );
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(
                          PinManager.pinDefini ? 'Defini' : 'Non defini',
                          style: TextStyle(
                            fontSize: 14,
                            fontWeight: FontWeight.w500,
                            color: PinManager.pinDefini ? kVert : kRouge,
                          ),
                        ),
                        const SizedBox(width: 8),
                        Icon(
                  ...
                  child: Row(
                    children: [
                      Icon(
                        ...
                        size: 20,
                        color: isDark ? kOrange : Colors.grey,
                      ),
                      const SizedBox(width: 14),
                      Expanded(
                        child: Text(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(
                          '${ContactsManager.contacts.length} contacts',
                          style: TextStyle(
                            fontSize: 14,
                            fontWeight: FontWeight.w500,
                            color: kSubtextCtx(context),
                ...
                GestureDetector(
                  onTap: () async {
                    final url = Uri.parse('https://haya.flexix.nl');
                    if (await canLaunchUrl(url))
                      await launchUrl(
                        url,
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                "Envoie. C'est parti.",
                style: TextStyle(fontSize: 15, color: kSubtextCtx(context)),
              ),
              const SizedBox(height: 40),
              Container(
                decoration: BoxDecoration(
